...
def test_tag_with_special_characters():
    assert create_tag_string_for_zim(["tag@name", "tag$name", "tag%name"]) == "@tagname @tag_name @tag_name"

def test_tag_with_quotes():
    assert create_tag_string_for_zim(["'tag name'", '"tag name"']) == "@tag_name @tag_name"
...
def test_mixed_tags_and_empty():
    assert create_tag_string_for_zim(["tag-name", "", "  spaced ", "hyphen-ated/slashed"]) == "@tag_name @spaced @slashed"

def test_tags_with_only_special_characters():
    assert create_tag_string_for_zim(["!@#$%^&*()"]) == ""

def test_tags_with_long_complex_mix():
    assert create_tag_string_for_zim(['complex/tag-name;with lots*of_stuff']) == "@tag_name_with_lots_of_stuff"

def test_tags_with_multiple_issues():
    assert create_tag_string_for_zim([" grand/child's-tag.name! "]) == "@childs_tag_name_"
